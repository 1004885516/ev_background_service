...
class TaskActiveKeep extends Subscription {
        ...
        for (let i = 0; i < packages.length; i++) {
            ...
            for (let j = 0; j < mslist.length; j++) {
                ...
                for (let k = 0; k < ms.Campaigns.length; k++) {
                    ...
                    const sql_new = `select 
                        count(distinct(DeviceId)) as count from ${usertable} 
                        join ${logintable} on ${usertable}.UserId = ${logintable}.UserId 
                        where ApkId = ? and date(Login_day) = ? ${Campaign.Sql}`;
            ...
            if (values_params.length > 0) {
                ...
                const sql = `insert into ${app.config.TableName.Keep_Active_Statistics} ${params_sql} values ?`;
            ...
            for (let m = 0; m < days.length; m++) {
                ...
                for (let j = 0; j < mslist.length; j++) {
                    ...
                    for (let k = 0; k < ms.Campaigns.length; k++) {
                        ...
                        const whereSql = `where date(Login_day) = ? and ApkId = ? ${Campaign.Sql} and ${logintable}.UserId in (SELECT ${logintable}.UserId FROM ${logintable} WHERE DATE(Login_day) = ?)`;
                        ...
                        const dayLoginWhereSql = `where date(Login_day) = ? and ApkId = ? ${Campaign.Sql}`;
                        const dayLoginSql = `${dayLoginSelectSql} ${joinSql} ${dayLoginWhereSql}`;
                        const dayLoginParams = [ day, apkId ].concat(Campaign.Params);
                ...
                const sql = `insert into ${app.config.TableName.Keep_Active_Statistics} ${params_sql} values ? on duplicate key update ${active_users} = values(${active_users})`;
                await ctx.mysqlQueryByLocal(sql, [ values_params ]);
            }
        }
    }
