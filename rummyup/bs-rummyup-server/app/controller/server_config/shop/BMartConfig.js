...
class RummyController extends Controller {
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.shop.NotifyGameServerByBMart(uploadData);
            if (result) {
    ...
    async getServerConfig() {
        const { ctx, app } = this;
        let bmartConfig = {};
        const result = await ctx.httpGet(app.config.GetShopBMartConfig);
        if (result) {
            ...
            bmartConfig.ChannelCode = config.ChannelCode;
            bmartConfig.Key = config.Key;
    ...
    jsonToArray(fileJson) {
        ...
        // 第一页
        // MinPayout MaxPayout PayoutCount Retained Tax ChannelCode	Key
        for (let i = 0; i < fileJson[0].length; i++) {
            ...
            bmartConfig.ChannelCode = item.ChannelCode;
            bmartConfig.Key = item.Key;
        }
