...
class RummyController extends Controller {
    ...
    async saveByOldLocal(saveData) {
        ...
        let sql = 'DELETE FROM sessions_rummy';
        ...
        for (let i = 0; i < saveData.length; i++) {
            ...
            dataParams += `(${data.session_id},${JSON.stringify(data.session_name)},${ctx.changeServerRatio(data.threshold)},
                ${ctx.changeServerRatio(data.base_score)},${ctx.changeServerRatio(data.min_into)},
                ${ctx.changeServerRatio(data.max_into)},${data.service_charge},${data.robot},`;
            for (let j = 0; j < 12; j++) {
        ...
        sql = 'INSERT INTO sessions_rummy ' +
            '(session_id, session_name, threshold, base_score, min_into, max_into, service_charge, robot,' +
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.session.notifyGameServerBySR(uploadData);
