...
class HighestJokerController extends Controller {
    async index() {
        const { ctx } = this;
    ...
    async uploadFileSessionHighestJoker() {
        const { ctx } = this;
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.session.notifyGameServerByHighestJoker(uploadData);
...
module.exports = HighestJokerController;
