...
class LowestJokerController extends Controller {
    async index() {
        const { ctx } = this;
    ...
    async uploadFileSessionLowestJoker() {
        const { ctx } = this;
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.session.notifyGameServerByLowestJoker(uploadData);
...
module.exports = LowestJokerController;
