...
class TwoPersonsRummyController extends Controller {
    async index() {
        const { ctx } = this;
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.session.notifyGameServerBySTR(uploadData);
    ...
    async getTemplateList() {
        const { ctx, app } = this;
        const filelist = await ctx.service.session.getSessionFiles(app.config.UploadFileType.TwoPersonsRummy);
        let templatefiles = [];
        if (filelist.length > 0) {
            for (let i = 0; i < filelist.length; i++) {
                const item = filelist[i];
                templatefiles.push({ filename: item.filename, createtime: ctx.transferDateTime(item.createtime), path: app.config.TwoPerFilePath });
            }
        } else {
            templatefiles.push({ filename: app.config.TPRTemplateName, path: app.config.TemplatePath });
        }
...
module.exports = TwoPersonsRummyController;
