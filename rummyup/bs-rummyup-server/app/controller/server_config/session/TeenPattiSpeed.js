...
class TeenPattiSpeedController extends Controller {
    async index() {
        const { ctx } = this;
    ...
    async uploadFileSessionTeenPattiSpeed() {
        const { ctx } = this;
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.session.notifyGameServerByQuickTP(uploadData);
...
module.exports = TeenPattiSpeedController;
