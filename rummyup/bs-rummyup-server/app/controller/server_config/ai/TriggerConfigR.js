...
class TriggerConfigRController extends Controller {
    async index() {
        const { ctx } = this;
        const serverData = await ctx.service.ai.getTriggerRConfig();
    ...
    async form() {
        ...
        if (uploadData) {
            let result = await ctx.service.ai.setTriggerRConfig(uploadData);
    ...
    jsonToArray(fileJson) {
        ...
        for (let i = 0; i < fileJson[0].length; i++) {
            ...
            data.PayinRank = item.PayinRank;
            data.PayinLevel = item.PayinLevel;
...
module.exports = TriggerConfigRController;
